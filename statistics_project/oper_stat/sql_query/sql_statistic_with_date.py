...
# campaign на base
count_success_call = """
SELECT c.operator, c.campaign, COUNT(*) AS successful_calls_count
FROM call c
JOIN base b ON c.campaign = b.campaign
WHERE c.success = true AND b.received_date BETWEEN %s AND %s
GROUP BY c.operator, c.campaign;
"""
...
WITH daily_calls AS (
    ...
    FROM call c
    JOIN base b ON c.campaign = b.campaign
    WHERE b.received_date BETWEEN %s AND %s
...
SELECT c.operator, CAST(AVG(COALESCE(c.duration, 0)) AS FLOAT) AS average_duration
FROM call c
JOIN base b ON c.campaign = b.campaign
WHERE c.success = TRUE AND b.received_date BETWEEN %s AND %s
GROUP BY c.operator;
...
SELECT c.operator, CAST(AVG(c.duration) AS FLOAT) AS average_duration
FROM call c
JOIN base b ON c.campaign = b.campaign
WHERE c.success = FALSE AND b.received_date BETWEEN %s AND %s
GROUP BY c.operator;
"""
